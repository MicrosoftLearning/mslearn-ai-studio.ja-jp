...
1. まだ有効になっていない場合は、ページ上部のツール バーで **[新しい Foundry]** オプションを有効にします。 次に、新しいプロジェクトを作成するための画面が表示された場合は、一意の名前を指定して作成します。このときに **[高度なオプション]** 領域を展開して、プロジェクトの設定を次のとおりに指定します。
    ...
    > \* この記事の執筆時点において、これらのリージョンでは、gpt-5 モデルの微調整をサポートしています。 各リージョンの最新の提供状況は、[モデルのページ](https://learn.microsoft.com/azure/foundry/foundry-models/concepts/models-sold-directly-by-azure?&pivots=azure-openai#fine-tuning-models)をご確認ください。

1. プロジェクトが作成されるまで待ちます。 次に、そのホーム ページを表示します。

## モデルをデプロイする

次は、パフォーマンスのベースラインを取得するために使用するモデルをデプロイします。

1.  これで、モデルを探索する準備ができました。 **[検出]** ページで **[モデル]** タブを選択して Microsoft Foundry モデル カタログを表示します。
1. モデル カタログで、`gpt-5` を検索します。
1. モデル カードを確認し、既定の設定を使用してデプロイします。
1. モデルがデプロイされると、モデル プレイグラウンドにそれが開きます。

## モデルの微調整

モデルの微調整は完了するまでに時間がかかるので、今すぐに微調整ジョブを開始し、既にデプロイした gpt-5 基本モデルを調べてから戻ります。
...
微調整ジョブの完了を待っている間に、*gpt-5* 基本モデルとチャットをして、そのパフォーマンスを評価しましょう。

1. 左側のペインで **[デプロイ]** を選択し、前にデプロイした **[gpt-5]** 基本モデルを選択します。
