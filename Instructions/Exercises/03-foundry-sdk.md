...
1. Web ブラウザーで [Microsoft Foundry](https://ai.azure.com) ポータル (`https://ai.azure.com`) を開いてビルドを開始します。Azure の資格情報を使ってサインインします。 初めてサインインしたときに開かれたヒントまたはクイック スタート ペインを閉じます。
...
1. **[エクスプローラー]** ペインの **labfiles/foundry-chat/python/chat-app** フォルダーで、**.env** ファイルを選択して開きます。 次に、構成値を更新して、**Azure OpenAI エンドポイント**と **gpt-5.2** モデルのデプロイに割り当てられた名前を含めます。
...
1. **Get a response (応答を取得する)** コメントの下のコードを変更し、要求時に前回の応答 ID を渡すようにします。その後、新しい応答 ID を取得して次回に追加できるようにします。
   ...
   last_response_id = response.id
    ...
    この手法を使用すると、前の応答の ID を渡してコンテキストを維持することができます。 また、会話を誘導したり、以前の会話スレッドを再開したりするために、任意の以前の応答から ID を渡す、より複雑なロジックを実装することもできます。
...
1. 会話を続けるには、次のプロンプトを入力します。
    ...
    今回は、ELIZA チャットボットと最新の LLM の比較を使ってアプリが応答する必要があります。 応答時間は非常に長くなる場合があり、アプリはモデルからすべて受信するまで待ってから表示します。そのため、アプリが応答していないように見えることがあります。 次は、これを修正しましょう。
