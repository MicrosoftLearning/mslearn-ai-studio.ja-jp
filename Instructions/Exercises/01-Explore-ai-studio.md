...
1. チャット ウィンドウで、以下のように「`Decribe three key considerations for working with Large Language Models for AI application development.`」などのクエリを入力し、応答を確認します。
...
1. キー、プロジェクト エンドポイント、および Azure OpenAI エンドポイントは書き留めておいてください。
    ...
    - "プロジェクト エンドポイント" は、OpenAI の**Resources** API を使用して Foundry で直接提供されるモデル (OpenAI モデルを含む) にアクセスしたり、Foundry 固有の API (Foundry Agent サービスなど) にアクセスしたりするために使用されます。**
    - "OpenAI エンドポイント" は、**Chat Completions** API、**Resources** API といった OpenAI の API を使用してモデルにアクセスするために使用されます。**

## Visual Studio Code 用に AI Toolkit 拡張機能をインストールする

開発者は、Foundry ポータルである程度の時間作業しますが、Visual Studio Code でも多くの時間を費やす可能性があります。 Microsoft Foundry の拡張機能は、開発環境を離れることなく Foundry プロジェクトのリソースを操作するための便利な方法を提供します。
