...
1. `gpt-5.2` モデルを検索し、検索結果から選択してそのモデル カードを表示します。

    モデル カードは、モデルの機能と制限事項を理解し、要件に適しているかどうかを判断するのに役立つモデルに関する情報を提供します。

    ![gpt-5.2 モデル カードのスクリーンショット。](../media/gpt5.2-details.png)
...
1. Foundry ポータルの上部のメニュー バーで、**[管理]** を選択します。

    管理センターでは、プロジェクトとその親リソースを表示したり、管理したりできます。

    ![Foundry ポータルの [管理] タブのスクリーンショット。](../media/ai-foundry-manage.png)
...
1. 拡張機能をインストールした後、左側のナビゲーション バーで **[Foundry Toolkit]** ページを選択し、読み込まれるまで待ちます。
...
1. 既定のプロジェクトを設定した後、プロジェクトを展開し、**[モデル]** を展開して、前にデプロイした **[gpt-5.2]** モデルを選択します。

    モデル デプロイの詳細については、こちらをご覧ください。

    ![Foundry Toolkit for Visual Studio Code 拡張機能のモデルのスクリーンショット。](../media/vscode-extension-model.png)

1. [Foundry Toolkit] ペインの **[開発者ツール]** セクションで、**[ビルド]** を展開し、**[モデル プレイグラウンド]** を選択します。 次に、**[gpt-5.2]** モデルを選択します (まだ選択されていない場合)。
