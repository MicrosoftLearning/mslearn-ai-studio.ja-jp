...
# 言語モデルを選択してデプロイする (非推奨)

> **注**: この演習は非推奨です。 代わりの演習 (<https://go.microsoft.com/fwlink/?linkid=2354118>) を行うことを検討してください。
...
1. **[+ 比較するモデル]** ボタンを使用して、**GPT-4o** モデルを一覧に追加します。 次に、同じボタンを使用して、**Phi-4-reasoning** モデルを一覧に追加します。
...
1. **X 軸**ドロップダウン メニューの **[品質]** で、次のメトリックを選択し、結果の各グラフを観察してから次に切り替えます。
    ...
    ベンチマークによれば、Phi-4-reasoning モデルは全体的なパフォーマンスが一番高いものの、コストが低いように見えます。
...
モデルを使用するには、Foundry *project* を作成する必要があります。
...
プロジェクトを作成すると、選択した **gpt-4o** モデルが自動的にデプロイされました。 検討した ***Phi-4-reasoning** モデルもデプロイしてみましょう。

1. 左側のナビゲーション バーの **[マイ アセット]** セクションで、**[モデル + エンドポイント]** を選択します。
1. **[モデル デプロイ]** タブの **[+ モデルのデプロイ]** ドロップダウン リストで、**[基本モデルのデプロイ]** を選択します。 次に、`Phi-4-reasoning` を検索して選択を確認します。
1. モデル ライセンスに同意します。
...
1. ナビゲーション バーで、**[プレイグラウンド]** を選択します。 次に、**[プレイグラウンドで開く]** を選択します。
1. チャットプレイグラウンドの **[設定]** ウィンドウで、**Phi-4-reasoning** モデルが選択されていることを確認し、チャット ボックスで最初の行を `System message: You are an AI assistant that helps solve problems.` として指定します(gpt-4o モデルのテストに使用したものと同じシステム プロンプトですが、システム メッセージのセットアップがないため、コンテキストの最初のチャットで指定します)。
