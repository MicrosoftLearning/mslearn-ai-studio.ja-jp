...
3. **[プロジェクトの作成]** ウィザードで、適切なプロジェクト名 (`my-ai-project` など) を入力し、既存のハブが推奨された場合は新しいハブを作成するオプションを選択します。 次に、ハブとプロジェクトをサポートするために自動的に作成される Azure リソースを確認します。
...
6. PowerShell ペインで、次のコマンドを入力して、この演習用のコード ファイルが入ったリポジトリをクローンします。
