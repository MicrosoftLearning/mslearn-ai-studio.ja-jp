...
> **注**: この演習は非推奨です。 代わりの演習 (<https://go.microsoft.com/fwlink/?linkid=2353647>) を行うことを検討してください。
...
1. 左のナビゲーション ウィンドウの下部で **[管理センター]** を選択します。 管理センターでは、*リソース* レベルと*プロジェクト* レベルの両方で設定を構成できます。これらはどちらもナビゲーション ウィンドウに表示されます。
    ...
    "リソース" レベルは、プロジェクトをサポートするために作成された **Foundry** リソースに関連します。** このリソースには、Azure AI サービス モデルと Foundry モデルへの接続が含まれており、AI 開発プロジェクトへのユーザー アクセスを一元的に管理するための場所となります。

    *プロジェクト* レベルは個々のプロジェクトに関連します。プロジェクトでプロジェクト固有のリソースを追加および管理できます。

1. ナビゲーション ウィンドウの Foundry リソースのセクションで、**[概要]** ページを選択してリソースの詳細を表示します。
...
1. [Azure portal] タブを閉じて、Foundry ポータルに戻ります。
...
1. チャット ウィンドウで、以下のように「`What are the key events in the history of Scotland?`」などのクエリを入力し、応答を確認します。
    ...
    ![Foundry ポータルのプレイグラウンドのスクリーンショット。](./media/ai-foundry-playground.png)
