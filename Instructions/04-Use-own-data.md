...
1. **[カスタマイズ]** を選択し、ハブに次の設定を指定します。
    ...
    > \* Azure OpenAI リソースは、リージョンのクォータによってテナント レベルで制限されます。 クォータ制限に達していて、両方のモデルに推奨されるリージョンがない場合は、そのうちの 1 つだけを選択し、その推奨リージョンを使用します。 演習の後半で、2 つ目のモデル用に、別のリージョンに別のリソースを作成します。
...
1. 前の手順を繰り返して、TPM レート制限が 5K である既定のバージョンの**標準**デプロイにより、`gpt-4` というデプロイ名で **GPT-4** モデルをデプロイします。
