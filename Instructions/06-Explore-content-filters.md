...
1. **[作成]** を選択し、選んだ Phi-4 モデル デプロイを含むプロジェクトが作成されるまで待ちます。
...
1. **[入力フィルター]** タブで、入力プロンプトに適用される設定を確認しします。
    ...
    フィルターは、フィルターでインターセプトされ阻止される特定の種類の言語を判別するために使用される **[ブロック (低)]**、**[ブロック (中)]**、**[ブロック (高)]** の各重大度のブロックしきい値に基づいて、これらのカテゴリごとにプロンプトと入力候補に適用されます。

    さらに、*プロンプト シールド*保護は、生成 AI アプリを悪用する意図的な試みを軽減するために提供されます。

1. 入力フィルターの各カテゴリのしきい値を **[ブロック (低)]、[ブロック (中)]、[ブロック (高)]** のいずれかに変更します。

1. **[出力フィルター]** ページで、出力応答に適用できる設定を確認し、各カテゴリのしきい値を **[ブロック (低)]、[ブロック (中)]、[ブロック (高)]** のいずれかに変更します。
