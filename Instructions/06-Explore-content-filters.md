...
1. **[チャット プレイグラウンド]** を開き、**[チャット履歴]** セクションに次のプロンプトを入力します。
...
1. **[チャット履歴]** セクションで、次のプロンプトを再入力します。

    ```
   Describe characteristics of Scottish people.
    ```

8. 出力を観察します。これは、人種差別的で見下すように求める要求はサポートされないと示すことが期待されます。 この不快な出力の防止は、コンテンツ フィルターの結果です。
